...
peer lifecycle chaincode package airmiletoken.tar.gz --path ${PWD}/../Chaincode/KBA-Automobile --lang node --label airmiletoken_1.0
sleep 1
...
# peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.airmile.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0 --collections-config ../Chaincode/KBA-Automobile/collection-automobile.json --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
# sleep 2
...
# peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.airmile.com --channelID $CHANNEL_NAME --name KBA-Automobile --version 1.0 --sequence 1 --collections-config ../Chaincode/KBA-Automobile/collection-automobile.json --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $AIRLINES_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $MERCHANTS_PEER_TLSROOTCERT --peerAddresses localhost:11051 --tlsRootCertFiles $BANKS_PEER_TLSROOTCERT --peerAddresses localhost:12051 --tlsRootCertFiles $CUSTOMERS_PEER_TLSROOTCERT
# sleep 1
